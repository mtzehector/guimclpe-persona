...
import mx.gob.imss.dpes.personafront.restclient.RegistroFrontClient;
 ...
 */
@Provider
public class RegistroFrontService extends ServiceDefinition<PersonaOrModel, PersonaOrModel> {

    @Inject
    @RestClient
    private RegistroFrontClient client;

    @Override
    public Message<PersonaOrModel> execute(Message<PersonaOrModel> request) throws BusinessException {
        Response create = null;
        try {
            create = client.create(request.getPayload().getRegistroRequest());
        ...
        log.log(Level.INFO, ">>> RegistroFrontService create.getStatus(): {0}", create.getStatus());
        if (create.getStatus() == 200) {
            log.log(Level.INFO, ">>> RegistroFrontService !!!OK response");
